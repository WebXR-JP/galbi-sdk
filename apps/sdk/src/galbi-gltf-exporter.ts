...
import { CoreExporter } from "./core-exporter";
...
class GltfExporter extends CoreExporter {
	...
	collectResources(root) {
		...
		const { materials, buffers, entityMeshInstances, textures, lights } = resources;
		// Collect entities
		root.forEach(entity => {
		...
		resources.entities.forEach(entity => {
			...
			if (entity.light && entity.light.enabled) {
				resources.lights.push(entity.light);
			}
		});
		return resources;
	}
	...
	writeLights(resources, json) {
		if (resources.lights.length > 0) {
			// Initialize extensions
			json.extensions = json.extensions || {};
			json.extensions.KHR_lights_punctual = json.extensions.KHR_lights_punctual || { lights: [] };
			json.extensionsUsed = json.extensionsUsed || [];
			if (json.extensionsUsed.indexOf("KHR_lights_punctual") < 0) {
				json.extensionsUsed.push("KHR_lights_punctual");
			}

			resources.lights.forEach(light => {
				const lightData = {
					type: light.type === "omni" ? "point" : light.type, // Convert 'omni' to 'point'
					color: [light.color.r, light.color.g, light.color.b],
					intensity: light.intensity,
				};
				// Add range for point and spot lights
				if (light.type === "omni" || light.type === "spot") {
					lightData.range = light.range;
				}
				// Add inner and outer cone angles for spot lights, convert degree to radian
				if (light.type === "spot") {
					lightData.spot = {
						innerConeAngle: (light.innerConeAngle * Math.PI) / 180,
						outerConeAngle: (light.outerConeAngle * Math.PI) / 180,
					};
				}
				json.extensions.KHR_lights_punctual.lights.push(lightData);
			});
		}
	}
	...
	writeNodes(resources, json) {
		if (resources.entities.length > 0) {
			json.nodes = resources.entities.map(entity => {
				...
				// Add light index to node
				if (entity.light && entity.light.enabled) {
					const lightIndex = resources.lights.indexOf(entity.light);
	...
	writeTextures(resources, textures, json, options) {
		for (let i = 0; i < textures.length; i++) {
			// convert texture data to uri
			const texture = textures[i];
			const data = texture.getSource();
			const isRGBA = true;
			...
			var header = new Int32Array(data.buffer, 0, headerLengthInt);
			var fourCC = header[off_pfFourCC];
			var width = header[off_width];
			var height = header[off_height];
			var dataOffset = header[off_size] + 4;
			let format = null;
			console.log(fourCC, width, height, dataOffset);
			...
			GltfExporter.writeBufferView(resources, json, arrayBuffer);
			resources.buffers.push(arrayBuffer);
			const bufferView = resources.bufferViewMap.get(arrayBuffer);
	...
	buildJson(resources, options) {
		return new Promise(resolve => {
			const json = {
				...
				extensionsUsed: ["EXT_voyage_exporter"],
				// extensionsRequired: [
				// "EXT_voyage_exporter"
				// ],
				scenes: [
			...
			this.writeLights(resources, json); // Add writeLights before writeNodes
			this.writeNodes(resources, json, options);

			this.writeTextures(resources, resources.textures, json, options);
	...
	build(entity, options = {}) {
		...
		return this.buildJson(resources, options).then(json => {
			...
			const binaryDataLength = json.buffers.reduce((total, buffer) => Math.ceil(total + buffer.byteLength / 4) * 4, 0);
			...
			if (binaryDataLength > 0) {
				...
				glbView.setUint32(offset + 4, 0x004e4942, true); // BIN
